...
class App extends Functions
{
    ...
    private static function router()
    {
        ...
        //- 4 For default urls that require a name - e.g. Contact/success/{{username}}
        $router->add( '{lang}/{namespace}/{controller}/{action}/{name}' );

        //- 5 For default urls for the frontend
        $router->add( '{lang}/{controller}/{action}' );
        //- 6 For default urls that require a name - e.g. Contact/success/{{username}}
        $router->add( '{lang}/{controller}/{action}/{name}' );
        //- 7 For default urls that require an id and a ptitle - e.g. en/Blog/page/002541/my-first-dumas-post
        $router->add( '{lang}/{controller}/{action}/{id:\d+}/{ptitle}');

        //Backwards compatibility, before the introduction of translation.
        //- 2 For LINKS urls already in circulation that don't have the language selector - we give a default language => en
        ...
        //In case off
        //- 1 If url is empty this I handle with the default router, but in case off....lol...
        ...
        //$router->add( '{controller}/{action}/{name}' , [ 'controller' => 'Contact', 'action' => 'success' ] );
